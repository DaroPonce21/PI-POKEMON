...
async function pokemonsApi() {
    try {
        ...
        pokemones = await Promise.all(pokemones.map(async (pokemon) => {
            ...
            let imgPokemon = dataPokemon.data.sprites.other['official-artwork'].front_default
            return {
    ...
    if (name) {
        ...
        try {
            ...
            let pokemonBd = await Pokemon.findAll({
                where: { name: name.toLowerCase() }, include: {
                    model: Tipo,
                }
            })
        ...
        pokemones = pokemones.concat(await Pokemon.findAll({
            include: {
                model: Tipo,
            }
        }));
...
router.post('/pokemons', async (req, res) => {
    ...
    if (!img) img = 'https://www.pinpng.com/pngs/m/8-82850_poke-ball-png-pokeball-png-transparent-png.png'
